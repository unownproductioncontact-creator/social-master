...
- **⚠️ Pièges vécus à la config Supabase/Render (08/07/2026)** — trois erreurs rencontrées en vrai, dans l'ordre :
  1. `ECONNREFUSED 127.0.0.1:5432` dans les logs Render = `DATABASE_URL` absente/vide (le driver `pg` retombe sur localhost). Signature à reconnaître immédiatement.
  2. `SELF_SIGNED_CERT_IN_CHAIN` au boot (dans le hook d'instrumentation, via pg-boss `isInstalled`) = Node rejette le certificat du **Session pooler** Supabase. Deux subtilités vérifiées dans les sources de pg 8.22 : (a) `?sslmode=require` y est un alias de `verify-full` (vérification STRICTE — contrairement à libpq), et (b) dans `pg/lib/connection-parameters.js`, le parsing de `connectionString` **écrase** la config explicite (`Object.assign({}, config, parse(cs))`) — passer `ssl: {rejectUnauthorized:false}` à la Pool est donc SANS EFFET tant que l'URL contient `sslmode`. **Correctif acté (code, pas env var)** : `pgConnectionConfig()` dans [src/lib/db-ssl.ts](src/lib/db-ssl.ts) retire tous les paramètres SSL de l'URL puis fournit `ssl` explicite — utilisé par Prisma ET pg-boss, testé unitairement, indépendant du contenu de `DATABASE_URL`. (Le `prisma db push` du start lit l'URL brute : le moteur Rust de Prisma tolère ce certificat, pas de retrait nécessaire.)
  3. Variable Render collée avec nom tronqué (`OKEN_ENCRYPTION_KEY` sans le T) : ne crashe PAS au boot (le code ne lit `TOKEN_ENCRYPTION_KEY` qu'au moment de chiffrer un token OAuth, cf. `src/lib/crypto.ts`) — l'app aurait démarré normalement et planté seulement à la première connexion Instagram/TikTok. Vérifier les NOMS des clés d'env, pas seulement leurs valeurs.
- **Connexion Supabase actée** : type **Session pooler** obligatoire (host `aws-0-eu-west-1.pooler.supabase.com:5432`, user `postgres.<project-ref>`) — le seul compatible à la fois IPv4 (Render ne joint pas la Direct connection, IPv6-only) et mode session (requis par pg-boss : verrous consultatifs/prepared statements ; le Transaction pooler le casserait silencieusement). Projet Supabase : `jikmelvwvquqmhbhlalz`, région eu-west-1, Free plan.
...
4. ~~Pousser le repo sur GitHub~~ — **fait le 07/07/2026** : repo public `https://github.com/unownproductioncontact-creator/social-master` (branche `main`). `.env` jamais commité (vérifié via `git check-ignore`), seul `.env.example` (sans secrets) est trackable.
5. ~~Déployer sur Render + Supabase~~ — **fait le 08/07/2026, VÉRIFIÉ EN PROD** : app live sur `https://social-master-jitq.onrender.com` (service `srv-d96n6j9kh4rs73cctka0`, team "Unown Production"). `/api/healthz` → 200 `{"status":"ok"}`, toutes les routes publiques 200, logs confirmant `db push` en sync, Next.js ready et `[worker] pg-boss démarré (queues: publish, reconcile, token-refresh)`. Env vars posées : `DATABASE_URL` (Supabase Session pooler), `APP_URL` (l'URL onrender.com pour l'instant), `AUTH_SECRET`, `TOKEN_ENCRYPTION_KEY`. Meta/TikTok/R2/Telegram encore vides (fonctionnalités concernées échoueront proprement d'ici là).
6. Prochaines actions user, dans l'ordre : (a) ~~créer son compte sur l'app en prod~~ **fait le 08/07** ; (b) ~~app TikTok~~ **CRÉÉE le 08/07** (voir ci-dessous) — reste : app Meta, R2, Telegram ; (c) demander à l'ami le CNAME `scheduler.dokkanessentials.com` → `social-master-jitq.onrender.com` puis mettre à jour `APP_URL` ; (d) tester le flow réel, programmer un vrai post ; (e) faire le test empirique TikTok brouillon (§10) avant de préparer l'audit Direct Post.

**App Meta/Instagram — BLOQUÉE le 08/07** : impossible de valider le compte développeur Meta. Nécessite un compte Facebook ; le compte fraîchement créé (Mohamed El Ahmadi, `mohamedelahma@gmail.com`) est retenu par l'anti-fraude Meta — email confirmé mais l'étape « Verify account » du parcours développeur exige un SMS qui n'arrive jamais (2 numéros testés : +33781811319, +33781811318, tous deux « en attente de confirmation »). Option carte bancaire disponible seulement au 1er écran (avant tout envoi SMS). À retenter après refroidissement du compte (24-48h) ou avec un compte Facebook ancien/établi. **Meta ne lie PAS le compte dev au compte Instagram publié** — n'importe quel compte FB peut servir à gérer l'app.

**App TikTok — CRÉÉE le 08/07** : compte dev TikTok via email+PIN (`unownproduction.contact@gmail.com`, PAS de SMS — RAS). App « Social Master », ownership **Individual**, type **Other**, app id `7660230324231964693`, en Draft. Icône 1024×1024 générée par code (`~/Downloads/social-master-icon.png`, dégradé violet→bleu + avion « envoyer »). **Vérification de domaine faite** : méthode **URL prefix / signature file** (PAS Domain/DNS, qu'on ne contrôle pas sur onrender.com). Fichier `public/tiktokoJ22hPvFX6f3Fa3VXcCXyGb1kF2xa1BC.txt` (contenu `tiktok-developers-site-verification=oJ22hPvFX6f3Fa3VXcCXyGb1kF2xa1BC`) servi à la racine → **« property verified »** confirmé. ⚠️ Piège corrigé : le matcher de `src/proxy.ts` n'excluait pas `.txt` → la requête TikTok (non authentifiée) était redirigée en 307 vers /login ; ajout de `txt|ico` aux exclusions. **Reste sur l'app TikTok** : compléter Basic information (icône, catégorie, description), ToS `…/legal/terms` + Privacy `…/legal/privacy`, Platforms (Web), ajouter les produits Login Kit + Content Posting API, scopes `user.info.basic`/`video.upload`/`video.publish`, URI de redirection `…/api/oauth/tiktok/callback`, puis récupérer Client key/secret → env vars Render. Quand le domaine ami sera actif, il faudra re-vérifier le nouveau préfixe `scheduler.dokkanessentials.com` de la même façon.

**UptimeRobot configuré le 08/07** : monitor HTTP sur `https://social-master-jitq.onrender.com/api/healthz`, intervalle **5 min** (compte UptimeRobot existant du user ; monitor id 803470276). But : garder le service Render éveillé pour que le worker pg-boss tourne en continu et que les publications partent à l'heure (sinon spin-down après ~15 min d'inactivité en plan gratuit). ⚠️ Limite connue : ~750 h d'instance gratuites/mois **partagées sur tout le compte Render** (le user a d'autres services free : masterball-dashboard, pokemoha) — garder Social Master éveillé 24/7 consomme ~730 h ; si tension sur le quota ou décalages de publication observés, passer Social Master en Render Starter (~7 €/mois, always-on, hors quota gratuit).

**R2 opérationnel le 09/07 — deux pièges vécus à la mise en service** : (1) le SDK AWS v3 récent ajoute par défaut un checksum (`x-amz-checksum-crc32`) que R2 ne gère pas sur les URLs présignées → PUT bloqué côté navigateur en `TypeError: Failed to fetch` générique ; correctif `requestChecksumCalculation: "WHEN_REQUIRED"` sur le S3Client (src/lib/storage.ts). (2) Le token API R2 créé par le user était « Object Read only » → 403 AccessDenied sur les PUT, masqué lui aussi en erreur réseau générique par le navigateur (les réponses d'erreur R2 n'incluent pas les en-têtes CORS). Diagnostic décisif : rejouer le PUT présigné via curl (la vraie réponse HTTP apparaît) — l'URL signée complète se récupère via la trace réseau du navigateur. Token corrigé (« social-media », bucket social-master, Object Read & Write). Upload vérifié de bout en bout en prod : presign→PUT 200→complete 200→listing→suppression. CORS bucket : PUT autorisé pour l'origin onrender.com (penser à ajouter le futur domaine custom).
...
## 17. Refonte complète + publication en masse (09/07/2026) — livrée

Chantier mené en orchestration multi-agents (5 audits parallèles puis 8 lots L0-L8 sur 3 vagues, Sonnet pour l'UI/exécution cadrée, Opus pour moteur/architecture/sécurité, intégration/commits/vérifications centralisés). **136/136 tests verts, build propre, tout déployé.**

- **L0 Moteur** : `PostTarget.scheduledAt` (horaire effectif PAR CIBLE), `schedulePost(postId, base, tz, targetTimes?)` rétrocompatible ; `src/lib/quota.ts` (compteur TikTok 5 brouillons/24h → BLOCAGE explicite décidé par le user ; snapshot quota IG non bloquant, jamais en dur).
- **L1 Design system** : accent « Cyan électrique » choisi par le user sur aperçu interactif (sky-600/500, dégradé signature sky-500→600 sur CTA/marque), PageHeader/EmptyState/StatCard/Skeleton + loading.tsx, layout (auth) partagé.
- **L2 Mobile** : Sheet (drawer Base UI), header mobile sticky + hamburger (<lg), calendrier en vue agenda sous md, grilles composer/médiathèque adaptées. Desktop inchangé.
- **L3 Upload v2** : PUT XHR avec progression par fichier, file bornée (3 simultanés, `upload-queue.ts` pur testé), retry/annulation, miniatures vidéo client (canvas → `/api/media/[id]/thumbnail` → `thumbnailKey`), job quotidien `media-cleanup` (purge UPLOADING > 1h).
- **L4 Bulk serveur** : `scheduleManyPosts` (pré-check capacité bloquant, une transaction PAR item, rapport par ligne, nettoyage du brouillon orphelin si programmation échoue) + `computeTargetTimes` pur (offset 300s défaut / simultané / custom) + Server Action zod (dates ISO).
- **L5 Page /composer/bulk** : cartes par vidéo (TikTok+IG cochés par défaut), réglages du lot (offset décochable, application groupée légende/hashtags, espacement N min entre vidéos), compteur TikTok live, bandeau permanent « légende non transmise » + Copier par carte, résultats par carte. Répétition générale e2e locale : 3 vidéos → 3/3 programmées, offsets vérifiés en base.
- **L6 Affichage** : chips horaires par plateforme (dashboard/calendrier/historique), chevauchement de jours géré, bouton « Copier la légende » dans l'historique (format identique au worker).
- **L7 Durcissement** : rate-limit mémoire (login 8/15min, signup, presign 30/min + plafond 9 Go/user), HeadObject à `complete` (taille réelle corrigée en DB), en-têtes de sécurité globaux (CSP volontairement différée).
- **Infra tests** : `npm test` = `scripts/test.sh`, 3 groupes sur moteur `prisma dev` FRAIS et VÉRIFIÉ (db push en boucle) — seul remède fiable aux désyncs de protocole du moteur allégé (bind message/portal/ECONNRESET), y compris la contention avec le serveur dev (à toujours ARRÊTER avant les tests). `npm run test:vitest` = runner brut. Réchauffage du premier envoi pg-boss dans le beforeAll du test bulk.

**Limites connues restantes** (voir §14) : caption TikTok non transmise avant l'audit Direct Post (bandeau + bouton Copier en attendant) ; app Meta/Instagram toujours bloquée par l'anti-fraude (compte FB neuf) ; CNAME ami en attente ; publication réelle TikTok/IG jamais exercée contre de vrais tokens (test empirique §10 toujours à faire en premier).

## 18. Parité maquette « design v2 » (09/07/2026, après-midi) — livrée

Le user a constaté que le L1 du §17 n'avait appliqué que les COULEURS de la maquette validée (artifact accent-preview, option C), pas sa structure/densité — et qu'en plus toute l'app rendait en Times. Deuxième chantier multi-agents (1 Opus primitives → 5 agents pages en parallèle → 1 Opus revue adversariale), spec = le HTML de la maquette réextrait de la transcription et suivi au pixel.

- **Bug police (cause du rendu « vieux document »)** : `@theme inline { --font-sans: var(--font-sans) }` — référence CIRCULAIRE introduite par le L1 → font-family invalide → fallback Times du navigateur. Corrigé en `var(--font-geist-sans)` (commit 1c25bf7). ⚠️ Piège associé : le cache persistant de **Turbopack** a continué de servir l'ancien CSS même après redémarrage complet du serveur dev — `rm -rf .next` obligatoire pour purger.
- **Primitives** (src/components/ui + layout) : tokens clairs exacts de la maquette (fond #f7f7fa, bordures #e6e7ee/#d8dae4, textes #16171d/#4b4e5a/#8a8d99), `StatusBadge` (+ `postStatusTone`/`postStatusLabel`), `PlatformChip` (+ `platformLabel`), `StatCard` (label/value/delta/deltaTone), `BrandMark` (tuile dégradé + Send), `PageHeader` avec prop `actions`, Button primaire = dégradé signature, Card 10px sans ombre, sidebar/mobile-header/user-menu densifiés.
- **Dashboard reconstruit** (la page comparée par le user) : 3 StatCards réelles (programmées 72h + « N aujourd'hui », publiées 30j + « +N cette semaine », échecs), liste « Prochaines publications » avec vignettes 44px (miniature réelle ou placeholder play), chips par cible, badges ; mini-calendrier de la semaine (aujourd'hui accent, pips) ; carte Connexions compacte. Composants dans src/components/dashboard/.
- **⚠️ prisma dev & concurrence** : le `Promise.all` de 6 requêtes (dont includes imbriqués) du dashboard tue le moteur local (`P1017 ConnectionClosed`, cf. §15) de façon REPRODUCTIBLE → les requêtes du dashboard sont exécutées **séquentiellement** (coût prod négligeable). Ne pas « ré-optimiser » en Promise.all.
- **Correctif majeur issu de la revue** : calendrier (month-grid) et historique affichaient les heures via `format()` date-fns = fuseau du PROCESS (UTC sur Render !) → tout est passé à `formatInTimeZone(…, User.timezone, …)` comme le dashboard, y compris le regroupement par jour et « aujourd'hui ». Bug pré-existant au chantier, visible seulement en prod.
- Autres correctifs de revue : liens Next `<Link>` au lieu d'`<a>` dans les actions du header ; requête « publiées 30 j » dédupliquée ; `take: 60` sur les programmés ; contraste badge muted (AA) ; distinction « Révoqué » dans la carte Connexions ; chips du SchedulePanel = heure PAR CIBLE (`PostTarget.scheduledAt`) ; PageHeader empilé sur mobile.
- **Vérification** : e2e visuel local sur données seedées identiques à la maquette (5 posts, tous statuts), desktop + mobile 375 + drawer, toutes pages. Revue adversariale Opus : aucune régression fonctionnelle, scoping userId vérifié partout, Base UI conforme (pas d'asChild).
- **Panne d'orchestration à connaître** : deux agents Opus ont terminé leur travail mais échoué 5× à rendre leur rapport `StructuredOutput` (champs émis en balises XML au lieu du JSON) → workflow en échec APPARENT alors que les fichiers étaient bons sur disque. Réflexe : `git status` + transcription de l'agent avant de relancer quoi que ce soit ; consigne « JSON strict, tous les champs dans UN objet » ajoutée aux prompts.

## 19. Premier test TikTok RÉEL (09/07/2026) — flow inbox validé + bug de chunking corrigé

Le user a connecté son vrai compte TikTok et lancé une publication en masse en prod. **C'est le test empirique §10 #1, enfin exécuté.** Résultats :

- ✅ **Le mode brouillon (inbox) FONCTIONNE avec un vrai token** : une vidéo a atteint `SEND_TO_USER_INBOX` (statut app « Envoyé en brouillon TikTok »). Grande inconnue du projet levée. **⚠️ Point d'ergonomie à retenir/communiquer** : l'inbox ne dépose PAS la vidéo dans *Profil → Brouillons* mais dans la **boîte de réception / notifications** de l'app TikTok (« votre vidéo est prête à publier »). Le user l'a d'abord cherchée au mauvais endroit. Reste à confirmer (test §10 #1 complet) : la vidéo finalisée depuis l'inbox peut-elle être rendue PUBLIQUE malgré l'app non auditée ?
- 🐛 **Bug réel corrigé — découpage en chunks TikTok** ([src/lib/providers/tiktok.ts](src/lib/providers/tiktok.ts)) : les vidéos **> 64 Mo** échouaient en `invalid_param` (« TikTok a refusé les paramètres »). Cause : `computeChunkRanges` faisait un découpage naïf « 64 Mo + reste » alors que TikTok impose **`total_chunk_count = plancher(video_size / chunk_size)`** (les premiers chunks à 64 Mo pile, le DERNIER absorbant tout le reste, entre 64 et < 128 Mo). Ex. 100 Mo : on annonçait 2 chunks, TikTok en attend 1 → refus. En plus, `chunk_size` était déclaré = taille du fichier pour un mono-chunk (ex. 100 Mo > plafond 64 Mo). **Corrigé** : découpage au modèle plancher exact + `chunk_size = min(taille, 64 Mo)`. L'ancien test ne validait QUE la cohérence interne (segments jointifs, reliquat ≥ 5 Mo), jamais la règle `plancher()` de TikTok — d'où le bug passé inaperçu ; test réécrit pour asserter le modèle réel (balayage 65→400 Mo + cas 100/130/150/200 Mo + régression nommée). Les vidéos ≤ 64 Mo n'étaient PAS affectées (mono-chunk trivial).

## 20. UX de suppression (09/07/2026, suite du test réel)

Frictions remontées par le user en voulant supprimer des médias, corrigées :
- **Suppression média silencieuse** : [media-card.tsx](src/components/library/media-card.tsx) appelait `deleteMediaAsset` dans un `<form action>` en IGNORANT le `{ error }` retourné → échec muet. Désormais tout passe par un composant client avec toast.
- **Bouton « Supprimer » d'un post masqué hors brouillon** : [composer/[postId]/page.tsx](src/app/(app)/composer/[postId]/page.tsx) ne montrait le bouton que si `isDraft`, alors que `deletePost` gère déjà tout statut (dé-programme d'abord). Désormais visible sur TOUS les statuts.
- **Suppression média EN CASCADE (décision produit du user, 09/07 : « pouvoir supprimer un fichier même s'il est sur un post programmé, que ça annule puis supprime »)** : logique extraite dans [deleteMediaAssetForUser](src/lib/media-delete.ts) (testable sans session — 4 tests d'intégration dans scheduler.test.ts), la Server Action [deleteMediaAsset](src/lib/actions/media.ts) ne fait qu'auth + revalidate. Ne bloque PLUS un média utilisé, mais **ne détruit QUE les posts pas-encore-publiés** : SCHEDULED → dé-programmé (`unschedulePost` annule les jobs pg-boss) puis supprimé ; DRAFT → supprimé ; **PUBLISHED/PARTIALLY_PUBLISHED/FAILED → CONSERVÉS** (la suppression du MediaAsset détache le média via `PostMedia onDelete: Cascade`, l'historique/`platformPostId`/URL restent intacts). ⚠️ Correctif d'une revue : la 1re version supprimait AUSSI les posts publiés (perte d'historique) avec un message rassurant trompeur — corrigé. Ordre : delete posts → delete MediaAsset (DB) → deleteObject R2 **best-effort de storageKey ET thumbnailKey** (sinon la miniature fuitait dans R2). La médiathèque affiche « Utilisé · N post(s) » par carte (via `_count.postMedia`) ; confirmation « Dé-programmer et supprimer » avec message exact.
- Nouveau composant réutilisable [ConfirmDeleteButton](src/components/ui/confirm-delete-button.tsx) (client) : dialog Base UI de confirmation + toast sur `{ error }` / succès. Action serveur liée via `.bind(null, id)` passée en prop `onConfirm` (les actions qui redirigent, comme `deletePost`, naviguent directement). Vérifié en navigateur.

## 21. Économie d'heures Render : mise en veille nocturne (09/07/2026)

Le forfait **Render gratuit partage ~750 h-instance/mois sur TOUT le compte** (Social Master + masterball-dashboard + pokemoha). Garder Social Master éveillé 24/7 via UptimeRobot en consommait ~730 à lui seul → tension sur le quota (a probablement causé un déploiement auto qui n'est pas parti et a dû être forcé en « Manual Deploy »). Décision du user : **laisser le service dormir de 23h à 7h (heure de Paris)**, il ne publie rien la nuit.

- **Mécanisme** : ce qui garde le service éveillé, c'est le ping keep-alive toutes les 5 min (sinon spin-down Render après 15 min d'inactivité). Les **maintenance windows d'UptimeRobot sont payantes (7 €/mois)** → refusées. Remplacé par un **workflow GitHub Actions gratuit** ([.github/workflows/keepalive.yml](.github/workflows/keepalive.yml)) qui pingue `/api/healthz` toutes les 5 min **uniquement de 05:00 à 21:59 UTC = 07:00–23:59 Paris (CEST été)**. Le service dort donc la nuit. Best-effort (GitHub peut retarder/sauter des crons ; le poller pg-boss + la réconciliation rattrapent au réveil). Cron `*/5 5-21 * * *` ; `workflow_dispatch` pour tester à la main ; run vérifié vert.
- **⚠️ CONTRAINTE : ne rien programmer entre 23h et 7h.** Un post prévu dans ce créneau ne partirait qu'au réveil (~7h, réconciliation). Réveil à froid ~30–60 s → caler les posts du matin à **7h10+**.
- **Action user requise** : mettre le monitor **UptimeRobot `social-master` (id 803470276) en PAUSE** — sinon il continue de pinger 24/7 et annule l'économie. (Non fait par l'assistant : pas d'accès UptimeRobot.)
- Rafraîchissement des tokens et crons de ménage (storage-check 5h UTC, media-cleanup 6h UTC, token-refresh) : tolérants à un réveil un peu tardif (cadence quotidienne, grosses marges) — aucun risque.
- **Outillage** : `gh` (GitHub CLI v2.96) installé dans `~/bin/gh` (pas de Homebrew sur la machine → binaire arm64 des releases GitHub ; `~/bin` PAS ajouté au PATH, le classifieur a bloqué l'édition de `~/.zshrc`). Authentifié par le user (compte `unownproductioncontact-creator`, scopes `repo,workflow,gist,read:org`) → `gh auth setup-git` permet à `git push` de créer des fichiers `.github/workflows/*` (le PAT git seul n'avait pas le scope `workflow`).
- **Vérif de déploiement fiable** : `/api/healthz` renvoie désormais `commit` (7 car. de `RENDER_GIT_COMMIT`). `curl …/api/healthz` → comparer au HEAD local, fini de deviner via les empreintes d'assets (les changements de pages authentifiées n'ont aucun marqueur public). `null` en local.

## 22. Audit + durcissement du flux Instagram AVANT le 1er vrai token (09/07/2026 soir)

Le compte dev Meta étant bloqué, tout le provider Instagram ([src/lib/providers/instagram.ts](src/lib/providers/instagram.ts), 15 fonctions) n'avait **jamais tourné contre un vrai token** (seule la classification d'erreurs était testée). Audit multi-agents (3 dimensions, chaque finding re-vérifié contre la doc Meta EN LIGNE, réfutés écartés). **Corrigés** (avant de brancher les vraies clés, pour marcher du 1er coup) :

- 🔴 **CRITIQUE — `permissions` = chaîne CSV, pas tableau** : `exchangeCodeForShortLivedToken` recevait `"scope_a,scope_b"` et le passait tel quel à `grantedScopes String[]` (Prisma) → « Expected List, provided String » → **le compte ne pouvait jamais se connecter**. Corrigé : normalisation CSV→`string[]` dans le provider.
- 🔴 **CRITIQUE — classification d'erreur par regex sur le blob brut** : `classifyInstagramError` regexait tout le corps (fbtrace_id inclus) → un `fbtrace_id` contenant « 190 »/« 458 » → `ig_token_invalid` → **NEEDS_REAUTH à tort, publications du compte en pause**, non déterministe. Corrigé : `extractMetaError()` parse `error.code`/`error.error_subcode` en ENTIERS, match par égalité exacte. Test de régression fbtrace.
- 🟠 **MAJEUR — `/me` non déballé** : la doc renvoie `{data:[{...}]}`, le code lisait un objet plat (incohérent avec l'échange de token qui déballe) → `user_id`/`username` undefined → upsert rejeté. Corrigé : même déballage défensif `data[]`.
- 🟠 **MAJEUR — polling container 5 s (12×/min)** vs reco Meta 1×/min → rafale sur carrousels multi-vidéos → risque rate-limit. Corrigé : défaut 30 s.
- 🟠 **MAJEUR — container `ERROR`/`EXPIRED` étiqueté « pas prêt » (2207027)** → retenté 3× (~21 min) sur un média non conforme + message trompeur. Corrigé : marqueurs distincts `ig_container_error` (→ content_rejected, pas de retry), `ig_container_expired`/`ig_container_timeout` (→ transitoire).
- 🟠 **§6.9 — erreur API brute non journalisée** : `markFailure` ne gardait que le message FR ; l'objet Meta brut (code/subcode/fbtrace) était jeté → 1er run indébogable. Corrigé : `ActivityLog.detail.rawError` + log serveur des échecs transitoires.
- 🟡 Mineurs : `GRAPH_BASE` épinglé `/v25.0` (évite la dérive de version par défaut) ; Story sans `caption` (non rendu par l'API) ; fallback quota `config.quota_total` → **25** conservateur au lieu de 100 fail-open.

**Réfutés par la re-vérification (NON modifiés, comportement correct)** : code 4 = spam (pas rate-limit) pour la variante Instagram (la réf officielle Instagram le confirme) ; « Meta exige de logger fbtrace » (faux — mais le §6.9 projet justifie quand même le log, cf. ci-dessus).

**Tests** : `src/lib/providers/instagram.test.ts` (nouveau, `fetch` mocké) exerce enfin le flux jamais couvert — permissions CSV→tableau, déballage /me, séquence Reel container→poll→publish→permalink, conteneur ERROR, carrousel enfants→parent, quota. `errors.test.ts` réécrit avec de vrais payloads JSON Meta + régression fbtrace. **⚠️ Toujours à faire une fois le token réel branché** : le VRAI 1er run (la forme exacte de `/me` — enveloppée vs plate — reste à confirmer sur l'API live ; le déballage gère les deux).

## 23. Soumission TikTok Production — vérif URL + piège « Save tout-ou-rien » (10/07/2026)

Préparation de la soumission Production TikTok (au-delà du sandbox amis). Le formulaire Production est **séparé** du Sandbox et repart quasi vide.

- ✅ **URL prefix vérifié côté Production** : `https://social-master-jitq.onrender.com/` (avec slash final) → « property has been verified ». C'est le **même fichier signature** déjà en ligne (`public/tiktokoJ22hPvFX6f3Fa3VXcCXyGb1kF2xa1BC.txt`, 200 en prod) — TikTok le réutilise, **rien à ré-uploader ni redéployer**.
- 🐛 **Piège slash final** : le champ « Web/Desktop URL » contenait `…onrender.com` **sans** slash → « This URL is not verified » (une URL sans slash n'a PAS le préfixe `…onrender.com/` comme sous-chaîne de tête). Corrigé en `…onrender.com/`.
- 🔴 **DÉCOUVERTE EMPIRIQUE MAJEURE — le « Save » du formulaire Production TikTok est TOUT-OU-RIEN** : il ne persiste **RIEN** tant que le formulaire n'est pas 100 % valide (tous les champs requis + products + scopes + vidéo démo). Vérifié : Description remplie → Save (formulaire incomplet) → reload → **Description revenue vide**. Seul le **nom** de l'app est réellement enregistré (depuis sa création). ⇒ **Remplir au fur et à mesure est inutile** ; il faut TOUT saisir en **une seule session**, la **vidéo démo étant le point bloquant** (à enregistrer par le user : flux sandbox login→upload→inbox, sur le domaine `onrender.com`, mp4/mov ≤ 50 Mo). ⚠️ Ne pas re-remplir le formulaire à blanc dans une session future en croyant que ça se sauvegarde.
- **Pour des amis (≤ 10) : la Production n'est PAS nécessaire.** Le **sandbox** (déjà configuré, `@dokkanessentials` ajouté) permet le vrai flux d'upload immédiatement, sans revue. Production = seulement pour > 10 users / public (revue ~2–4 semaines).
- **Kit prêt-à-coller complet** (valeurs exactes : Category/Description/ToS/Privacy/URL, usage description EN, products `Login Kit`+`Content Posting API`, scopes `user.info.basic`+`video.upload`, script vidéo démo, ordre des opérations) → **`~/tiktok-production-kit.md`** (hors repo). `video.publish`/Direct Post toujours déféré.

## 24. Audit complet + 22 corrections en 3 vagues d'agents (10-11/07/2026) — livré

Audit multi-agents (6 angles + contre-vérification adversariale) → rapport **`~/Audit-SocialMaster-2026-07-10.md`** (22 constats retenus, preuves fichier:ligne). Le user a validé « toutes les corrections par ordre de priorités » → 3 vagues d'agents (Opus = moteur/schéma, Sonnet = UI), lots découpés PAR FICHIER (zéro conflit), intégration centralisée entre vagues (tsc + `npm test` 3 groupes + build). Commit `cf2e509`, déployé. **Exclusion actée : la fusion complète composer/bulk (P2-1) n'a PAS été faite** — seulement l'alignement (miniatures/défauts/upload) ; la fusion reste une décision produit future.

**Nouveaux modules** (tous purs et testés) : `src/lib/schedule-window.ts` (fenêtre morte 23h–7h : isInQuietWindow/suggestWakeTime/QUIET_WINDOW_LABEL), `src/lib/tiktok-window.ts` (maxCountInSlidingWindow — plafond TikTok en fenêtres 24h GLISSANTES, client-safe), `src/lib/last-used.ts` (mémoire localStorage hashtags/heure), `src/lib/media-retention.ts` (sélection pure des médias purgeables), `src/components/util/auto-refresh.tsx` (router.refresh périodique onglet visible — monté sur dashboard/historique/détail).

**Changements de COMPORTEMENT moteur à connaître** :
- `unschedulePost` ne réinitialise QUE les cibles ∉ {PUBLISHED, SENT_TO_INBOX} → **un post DRAFT peut légitimement porter des cibles déjà publiées** (post partiellement publié « repassé en brouillon »). `savePostDraft` ne supprime/recrée jamais ces cibles (plateforme « déjà servie » = case verrouillée dans l'UI d'édition). `schedulePost` n'enfile des jobs que pour PENDING/FAILED. `unschedulePostAction` refuse PUBLISHED.
- Nouvelle action `reschedulePostAction({postId, scheduledAtLocal, timezone})` : reprogrammation en 1 étape (SCHEDULED uniquement), préserve les décalages relatifs entre cibles.
- **Pré-check TikTok fenêtré** : le blocage 5 brouillons/24h se calcule en fenêtres glissantes sur les horaires RÉELS (lot multi-jours plus jamais bloqué à tort) ; le check mono-post est ancré sur l'horaire du post (quota.ts `checkTikTokDraftCapacityAt` + `getTikTokDraftEventTimes`). `getBulkQuotaInfo` renvoie aussi `tiktokEventTimesMs` pour le compteur client (« Journée la plus chargée : X/5 »).
- **Telegram** : notif à chaque dépôt inbox TikTok (légende complète prête à coller + lien /composer/{id}) ; notif d'échec avec lien. (Worker → redémarrage complet requis en dev.)
- `User.mediaRetentionDays Int?` (nouvelle colonne, `db push` au start prod) : purge quotidienne optionnelle (Paramètres → Jamais/7/30/90 j) des médias dont TOUS les posts sont PUBLISHED depuis N jours (jamais les médias non utilisés) — 2e passe dans media-cleanup-job.
- Quota IG : timeout AbortSignal (8s défaut, 3s sur le rendu de la page bulk).

**Contrats d'URL inter-pages** (ne pas casser) : `/composer?media={id}` (pré-sélection), `/composer?date={yyyy-MM-dd}` (pré-rempli à 18h, utilisé par le « + » du calendrier), `/composer?duplicate={postId}` (nouveau brouillon pré-rempli, bouton « Dupliquer » du détail) ; `/history?filter=upcoming|failed|tiktok&limit=50..300` (stat dashboard → `?filter=failed`).

**UI notable** : statut cible SENT_TO_INBOX = « À finaliser sur TikTok » (orange, plus jamais vert) ; dashboard = carte « À traiter » (échecs + brouillons TikTok à finaliser avec Copier) + carte « Premiers pas » si 0 compte + stat échecs cliquable ; composer mono = plateformes pré-cochées, programmation en 1 action (« demain 18h » + puces, heure habituelle mémorisée), miniatures réelles ; SchedulePanel par statut (PUBLISHED = lecture seule) + avertissement 23h–7h avec « Décaler à 7h10 » (composer, panel, bulk) ; bulk = brouillon de lot persistant (sessionStorage `bulk-draft-v1`), « Heure de départ » propagée aux cartes non éditées (`timeTouched`), fuseau = User.timezone ; médiathèque = sélection multiple + « Utiliser dans un post » + popover d'incompatibilités ; historique = filtres + « Afficher plus » ; upload → `/api/media/[id]/complete` renvoie `publicUrl` (type `UploadedMedia.publicUrl` OBLIGATOIRE pour tout futur appelant de onUploaded).

**Reste à vérifier en NAVIGATEUR par le user (non testé en réel, tsc/tests/build seulement)** : flux Reconnecter/Déconnecter sur un vrai compte ; « Modifier l'horaire » sur un post SCHEDULED ; encarts 23h–7h ; restauration sessionStorage du bulk après refresh ; sélection multiple médiathèque ; densité de la vue agenda mobile du calendrier (le « + » ajoute une ligne par jour vide — à ajuster si trop verbeux) ; notif Telegram inbox (nécessite une vraie publication TikTok).

## 25. YouTube Shorts — spec actée (11/07/2026, faits vérifiés docs officielles le jour même)

Troisième plateforme : **publication DIRECTE** (pas d'inbox), la plus simple des trois. Chantier en vagues d'agents comme §24.

**Faits API vérifiés (11/07/2026)** :
- Upload : `videos.insert` (YouTube Data API v3), **resumable** (init POST `https://www.googleapis.com/upload/youtube/v3/videos?uploadType=resumable&part=snippet,status` → header `Location` → PUT du binaire, streamé depuis R2). Max 256 Go. MIME `video/*`.
- **Quota : bucket DÉDIÉ de 100 videos.insert/jour (1 unité/appel)** + 10 000 unités/jour pour le reste (videos.list=1, videos.update=50). L'ancien modèle 1 600 unités/upload n'est plus en vigueur. Aucun garde-fou bloquant nécessaire à notre volume ; classifier l'erreur `quotaExceeded`/`uploadLimitExceeded` proprement suffit.
- **Un Short = classification AUTOMATIQUE** : vidéo verticale/carrée (≥ 1:1, idéal 9:16) ET ≤ 3 min (180 s). Aucun flag API, `#Shorts` optionnel. Horizontale ou > 3 min → vidéo classique (pas une erreur — juste pas un Short). Pas de miniature custom pour les Shorts.
- **Programmation native possible** : `status.publishAt` (« It can be set only if the privacy status of the video is private ») — YouTube publie lui-même à l'heure dite, même pendant la veille Render 23h-7h. **V1 : NON utilisée** (uniformité : pg-boss publie à H en `privacyStatus: public`, comme IG/TikTok). Évolution possible V2.
- Statut : la réponse d'upload donne l'id immédiatement → URL `https://www.youtube.com/shorts/{id}` ; `processingDetails.processingStatus` (processing/succeeded/failed) si besoin de poller.
- **OAuth** : scopes `youtube.upload` + `youtube.readonly` (channels.list mine=true pour nom/avatar de chaîne), `access_type=offline` + `prompt=consent` (garantit le refresh_token). Access token ~1 h → **refresh AVANT chaque publication** (pas de rotation du refresh token côté Google) ; le cron quotidien ne fait rien pour YOUTUBE en V1. Token endpoint `https://oauth2.googleapis.com/token`.
- **⚠️ LE piège : statut de publication de l'app Google Cloud.** `youtube.upload` = scope SENSIBLE. En statut « Testing » → **refresh tokens expirent en 7 jours** (inutilisable pour un planificateur). En « In production » NON vérifiée → écran « app non validée » à la connexion (contournable : Paramètres avancés), plafond 100 utilisateurs à vie, PAS d'expiration 7 jours. **Cible : production non vérifiée.**
- **Réutilisation du projet Google Cloud de Kiibiki** (scopes actuels : youtube.readonly + yt-analytics[-monetary].readonly, cf. ~/kiibiki/src/lib/oauthUrl.ts) : ajouter le scope youtube.upload à l'écran de consentement, vérifier/passer le statut en production, créer un 2e client OAuth « Social Master » (redirect `https://social-master-jitq.onrender.com/api/oauth/youtube/callback`) → `GOOGLE_CLIENT_ID`/`GOOGLE_CLIENT_SECRET` dans Render. Quotas partagés par projet : sans conflit (Kiibiki = lecture, bucket uploads dédié).

**Décisions produit actées** :
- Enums : `Platform.YOUTUBE`, `PostContentType.YOUTUBE_SHORT`, `publishMode: AUTO`. 1 vidéo exactement (pas de photo/carrousel YouTube en V1).
- **Titre requis par YouTube (≤ 100 car.)** : champ « Titre YouTube » dans le composer mono quand YouTube est coché, défaut = 1re ligne de la légende tronquée ; en bulk = auto (1re ligne), pas de champ par carte en V1. Stocké dans `PostTarget.platformOptions.title` (contrat : `{ title?: string }`).
- Description = légende + hashtags (même format que le presse-papiers/worker). `status.selfDeclaredMadeForKids: false` posé explicitement. Pas de categoryId.
- Mode offset du lot : TikTok à H, Instagram H+5 min, **YouTube H+10 min** ; mode custom : champ d'horaire YouTube par carte comme les deux autres.
- Validation média (avertissements, non bloquants sauf impossibilité) : vidéo obligatoire ; > 180 s ou ratio horizontal → avertissement « sera publiée comme vidéo classique, pas comme Short » ; formats déjà acceptés par l'app OK.
- Classification d'erreurs : `invalid_grant`/`unauthorized` → reauth (NEEDS_REAUTH) ; `quotaExceeded`/`uploadLimitExceeded`/`rateLimitExceeded` → quota (pas de retry, message FR « quota YouTube atteint, réessayez demain ») ; processing `failed`/`rejected` → content_rejected (jamais retenté) ; 5xx/réseau → transitoire (retry backoff existant).
- **Tests empiriques jour 1** (une fois les env vars posées) : (1) durabilité du refresh token en production non vérifiée (> 7 jours) ; (2) une vidéo verticale < 3 min atterrit bien dans l'étagère Shorts de la chaîne ; (3) plafond d'upload par CHAÎNE (indépendant de l'API, chaînes récentes plus limitées — chaîne établie du user a priori OK).

**LIVRÉ le 11/07/2026 (jalons J0→J3, commits 183e0d2/4c6d02c/821f8e6)** : moteur complet (OAuth routes + provider resumable streamé R2 + classifyYouTubeError + branche worker + health-check YOUTUBE refresh-d'abord + offsets H/H+5/H+10 + bulk serveur/zod) et UI complète (case « YouTube (Short) » + champ Titre ≤100 avec placeholder dynamique + warnings média + onglet preview + duplication ; bulk par carte + youtubeTime custom + sessionStorage v2 + fix applyWakeTime ; 3e carte Connexions + feedback ?youtube= ; ligne dashboard ; diagnostics médiathèque). Helpers titre UNIFIÉS dans content-type.ts (`resolveYouTubeTitle` = canonique, repli ultime « Short »). Tests : 251 purs + 13 scheduler + 20 bulk verts, build prod OK. **Reste J4 (actions user)** : scope youtube.upload + statut production sur le projet Google Cloud Kiibiki, 2e client OAuth, GOOGLE_CLIENT_ID/SECRET sur Render, connexion chaîne, tests empiriques jour 1 (étagère Shorts, refresh >7 j, plafond chaîne). Suivi de chantier : CHANTIER-YOUTUBE-JALONS.md.
