...
6. Prochaines actions user, dans l'ordre : (a) créer son compte sur l'app en prod ; (b) demander à l'ami le CNAME `scheduler.dokkanessentials.com` → `social-master-jitq.onrender.com`, puis mettre à jour `APP_URL` ; (c) créer l'app Meta + l'app TikTok (nécessite le sous-domaine actif) ; (d) créer le bucket R2 + bot Telegram, remplir les env vars ; (e) tester le flow Instagram réel, programmer un vrai post ; (f) faire le test empirique TikTok brouillon (§10) avant de préparer l'audit Direct Post.
