...
- **Suppression média EN CASCADE (décision produit du user, 09/07 : « pouvoir supprimer un fichier même s'il est sur un post programmé, que ça annule puis supprime »)** : [deleteMediaAsset](src/lib/actions/media.ts) ne bloque PLUS un média utilisé — il dé-programme (`unschedulePost` → annule les jobs pg-boss) puis SUPPRIME chaque post qui l'utilise (cascade Prisma pour PostTarget/PublishJob/PostMedia), avant de supprimer le fichier. La médiathèque affiche « Utilisé · N post(s) » par carte (via `_count.postMedia`) et la confirmation annonce combien de posts partiront (label « Dé-programmer et supprimer »). Le contenu déjà publié sur les plateformes n'est pas touché (on n'agit que sur nos données).
