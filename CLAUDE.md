...
- **⚠️ Pièges vécus à la config Supabase/Render (08/07/2026)** — trois erreurs rencontrées en vrai, dans l'ordre :
  ...
  2. `SELF_SIGNED_CERT_IN_CHAIN` au boot (dans le hook d'instrumentation, via pg-boss `isInstalled`) = Node rejette le certificat du **Session pooler** Supabase avec `?sslmode=require`. **Correctif acté : `?sslmode=no-verify`** en fin de `DATABASE_URL` (TLS chiffré, vérification du certificat désactivée — standard avec node-postgres + pooler Supabase). Prisma ET pg-boss lisent la même URL, donc un seul endroit à corriger.
