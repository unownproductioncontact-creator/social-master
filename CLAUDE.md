...
Deux frictions remontées par le user en voulant supprimer des médias, corrigées :
- **Suppression média silencieuse** : [media-card.tsx](src/components/library/media-card.tsx) appelait `deleteMediaAsset` dans un `<form action>` en IGNORANT le `{ error }` retourné — un média rattaché à un post refusait la suppression (garde-fou voulu, [media.ts:18](src/lib/actions/media.ts)) mais SANS aucun message → le user croyait à un bug. Désormais l'erreur s'affiche en toast.
- **Bouton « Supprimer » d'un post masqué hors brouillon** : [composer/[postId]/page.tsx](src/app/(app)/composer/[postId]/page.tsx) ne montrait le bouton que si `isDraft`, alors que `deletePost` gère déjà tout statut (il dé-programme d'abord). Il obligeait à « repasser en brouillon » avant de supprimer. Désormais visible sur TOUS les statuts.
- Nouveau composant réutilisable [ConfirmDeleteButton](src/components/ui/confirm-delete-button.tsx) (client) : dialog Base UI de confirmation + toast sur `{ error }` / succès. Utilisé par la carte média ET la page post. Une action serveur liée via `.bind(null, id)` est passée en prop `onConfirm` (les actions qui redirigent, comme `deletePost`, naviguent directement). Vérifié en navigateur : toast d'erreur « média utilisé » OK, bouton présent sur post en Échec, suppression + redirection OK.
